...
Only the real current month is writable on the home surface. When any non-current month is visible there, replace the check-in controls with a read-only monthly summary and do not expose record, edit, or clear actions. A completed showroom month may be supplemented only while unsealed; a sealed month is read-only until the user explicitly confirms an unseal. Completed-month statistics use the month's full day count; the current month uses only days through today; future months have no elapsed-day statistics. Future showroom stones must reuse their real image with CSS blur and darkening instead of a separate placeholder asset.

Status remains read-only in showroom month details, but an existing relapse mark may be tapped to view and edit its optional trigger metadata. This exception must never expose status changes, backfilling, or record deletion from the showroom.

Recording a day is an immediate-save ritual: persist the status before animation, then reveal the real carving overlay along its path with restrained stone chips, dust, warm surface light, haptic feedback, and optional stone sound. Respect `prefers-reduced-motion` by removing shake and particles. A relapse opens a nonjudgmental, skippable trigger sheet after the carving; allow up to three trigger categories and an optional 30-character note.

The secondary menu is grouped into Data, App, and Danger sections. Keep reset at the bottom in the Danger section, close the menu before presenting its confirmation, offer backup first, and require a second explicit confirmation before clearing records, trigger metadata, and month seals.
